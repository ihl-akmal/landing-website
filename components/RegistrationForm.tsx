...
type FormErrors = { [key in keyof RegistrationFormValues]?: string };
...
export default function RegistrationForm({ classId, closeDate }: RegistrationFormProps) {
  ...
  const handleNext = async () => {
    ...
    setErrors({}); // ← clear SEMUA error sebelum pindah step
    setStep(2);
};
  ...
  const handleFormSubmit = async () => {
      ...
      if (!result.success) {
          ...
          setErrors(newErrors);
          // Jika ada eror di Langkah 1, kembali ke Langkah 1
      ...
      try {
        const actionResult = await createRegistration({ ...result.data, classId });
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="mt-6 flex justify-end gap-4">
            ...
            {step === 1 ? (
                <Button type="button" onClick={handleNext} className="bg-gradient-to-r from-primary to-primary-light text-white font-semibold hover:shadow-lg transition-all duration-200 shadow-md">
                    Lanjut
                </Button>
            ) : (
