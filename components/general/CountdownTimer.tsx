      ...
      {hasTimeLeft ? (
        <div className='flex items-center justify-center gap-2 tracking-wider'>
            ...
            <span className="font-bold tabular-nums">
                {String(timeLeft.days).padStart(2)} hr : {String(timeLeft.hours).padStart(2)} j : {String(timeLeft.minutes).padStart(2, '0')} m : {String(timeLeft.seconds).padStart(2, '0')} d
            </span>
        </div>
      ) : (
        <span>SEAT HAMPIR HABIS! SEGERA AMANIN SEAT</span>
      )}
    </div>
  );
};
